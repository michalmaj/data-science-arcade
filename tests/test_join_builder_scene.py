...
JOIN_TYPE_OPTIONS = (
    JoinTypeOption("inner", "app.title", "inner"),
    JoinTypeOption("left", "common.on", "left"),
    JoinTypeOption("outer", "common.off", "outer"),
)
...
def _make_scene(app, left=None, right=None, context=None, on_complete=lambda choice, ok: None, **kwargs):
    return JoinBuilderScene(
        app,
        "app.title",
        left if left is not None else generate_orders(),
        right if right is not None else generate_customers(),
        "customer_id",
        JOIN_TYPE_OPTIONS,
        on_complete,
...
@pytest.mark.parametrize(
    "index,how,expected_total",
    [(0, "inner", JOIN1_INNER_ROW_COUNT), (1, "left", JOIN1_LEFT_ROW_COUNT), (2, "outer", JOIN1_OUTER_ROW_COUNT)],
)
def test_each_join_type_computes_the_real_row_count_live(index, how, expected_total):
    app = _init_app()
    try:
        scene = _make_scene(app)
...
def test_a_many_to_one_violation_is_caught_and_shown_as_a_distinct_state_not_a_crash():
    app = _init_app()
    try:
        raw_options = (JoinTypeOption("left_raw", "app.title", "left"),)
...
def test_a_caught_validation_failure_still_lets_the_scene_finish():
    ...
    try:
        ...
        scene = JoinBuilderScene(
            ...
            "customer_id",
            raw_options,
            lambda choice, ok: recorded.append((choice, ok)),
            context,
            validate="many_to_one",
        )
        scene.buttons.buttons[0].on_activate()
...
def test_a_real_cardinality_that_satisfies_validate_succeeds_normally():
    app = _init_app()
    try:
        promo_per_customer = generate_active_promotions().frame.groupby("customer_id", as_index=False).agg(
        ...
        right = Dataset(
            ...
            frame=promo_per_customer,
            schema=Schema(columns=(ColumnSchema("customer_id", "object"), ColumnSchema("active_promotion_count", "int64"))),
        )
        raw_options = (JoinTypeOption("left_repaired", "app.title", "left"),)
        scene = JoinBuilderScene(
            app, "app.title", generate_orders(), right, "customer_id", raw_options, lambda choice, ok: None, LessonContext(), validate="many_to_one"
        )
        scene.buttons.buttons[0].on_activate()
        merged, error = scene._attempt_merge()
        assert error is None
...
def test_a_wrong_group_key_does_not_overflow_the_preview_the_scene_still_draws_safely(headless_display=None):
    app = _init_app()
    try:
        scene = _make_scene(app)
