...
def _pick_every_definition_correctly(scene: FunnelBuilderScene) -> None:
    for _ in range(len(scene.requests)):
        request = scene._current_request()
        correct_key = CORRECT_DEFINITION_BY_REQUEST[request.key]
...
def test_the_full_lesson_plays_through_all_eight_stages_to_a_result():
    ...
    try:
        finished_results = []
        runner, collected = build_lesson_twenty_one_runner(
        ...
        assert isinstance(app.scenes.current.inner, FunnelBuilderScene)  # guided
        assert app.scenes.current.guided is True
        _pick_every_definition_correctly(app.scenes.current)

        assert isinstance(app.scenes.current.inner, DialogueScene)  # independent intro
        _play_dialogue_to_the_end(app.scenes.current)

        assert isinstance(app.scenes.current.inner, FunnelBuilderScene)  # independent
        ...
        assert result.guided_choices == CORRECT_DEFINITION_BY_REQUEST
        assert result.independent_choices == CORRECT_DEFINITION_BY_REQUEST
