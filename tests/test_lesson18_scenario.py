...
from data_science_arcade.lessons.l18_randomization_control_room.requests import CORRECT_RULE_BY_REQUEST
...
def _pick_every_rule_correctly(scene: SegmentSlicerScene) -> None:
    for _ in range(len(scene.requests)):
        request = scene._current_request()
        correct_key = CORRECT_RULE_BY_REQUEST[request.key]
...
def test_the_full_lesson_plays_through_all_eight_stages_to_a_result():
    ...
    try:
        finished_results = []
        runner, collected = build_lesson_eighteen_runner(
        ...
        assert isinstance(app.scenes.current.inner, SegmentSlicerScene)  # guided
        assert app.scenes.current.guided is True
        _pick_every_rule_correctly(app.scenes.current)

        assert isinstance(app.scenes.current.inner, DialogueScene)  # independent intro
        _play_dialogue_to_the_end(app.scenes.current)

        assert isinstance(app.scenes.current.inner, SegmentSlicerScene)  # independent
        ...
        assert result.guided_choices == CORRECT_RULE_BY_REQUEST
        assert result.independent_choices == CORRECT_RULE_BY_REQUEST
