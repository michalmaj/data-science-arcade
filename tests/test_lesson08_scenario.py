...
from data_science_arcade.lessons.l08_duplicate_detective.candidate_pairs import (
    CANDIDATE_PAIRS,
    CORRECT_DECISION_BY_PAIR,
)
from data_science_arcade.lessons.l08_duplicate_detective.scenario import DECISION_FIELDS, build_lesson_eight_runner
...
def _decide_every_pair_correctly(scene: RecordPairScene) -> None:
    for pair in CANDIDATE_PAIRS:
        decision = CORRECT_DECISION_BY_PAIR[pair.key]
...
def test_the_full_lesson_plays_through_all_eight_stages_to_a_result():
    ...
    try:
        finished_results = []
        runner, collected = build_lesson_eight_runner(
        ...
        assert isinstance(app.scenes.current.inner, RecordPairScene)  # guided
        assert app.scenes.current.guided is True
        _decide_every_pair_correctly(app.scenes.current)

        assert isinstance(app.scenes.current.inner, DialogueScene)  # independent intro
        _play_dialogue_to_the_end(app.scenes.current)

        assert isinstance(app.scenes.current.inner, RecordPairScene)  # independent
        ...
        assert result.guided_decisions == CORRECT_DECISION_BY_PAIR
        assert result.independent_decisions == CORRECT_DECISION_BY_PAIR
...
def test_every_candidate_pair_has_a_correct_decision_recorded():
    assert {pair.key for pair in CANDIDATE_PAIRS} == set(CORRECT_DECISION_BY_PAIR)
