...
from data_science_arcade.lessons.l14_chart_designer.requests import CORRECT_OPTION_BY_REQUEST
...
def _pick_every_chart_correctly(scene: ChartDesignerScene) -> None:
...
def test_the_full_lesson_plays_through_all_eight_stages_to_a_result():
    ...
    try:
        finished_results = []
        runner, collected = build_lesson_fourteen_runner(
        ...
        assert isinstance(app.scenes.current.inner, ChartDesignerScene)  # guided
        assert app.scenes.current.guided is True
        _pick_every_chart_correctly(app.scenes.current)

        assert isinstance(app.scenes.current.inner, DialogueScene)  # independent intro
        _play_dialogue_to_the_end(app.scenes.current)

        assert isinstance(app.scenes.current.inner, ChartDesignerScene)  # independent
