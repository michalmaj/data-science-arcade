...
from data_science_arcade.lessons.l25_kpi_emergency_room.requests import CORRECT_COMBO_BY_REQUEST
...
def _pick_every_combo_correctly(scene: AlertConfigScene) -> None:
    for _ in range(len(scene.requests)):
        request = scene._current_request()
        correct_metric, correct_threshold = CORRECT_COMBO_BY_REQUEST[request.key]
...
def test_the_full_lesson_plays_through_all_eight_stages_to_a_result():
    ...
    try:
        finished_results = []
        runner, collected = build_lesson_twenty_five_runner(
        ...
        assert isinstance(app.scenes.current.inner, AlertConfigScene)  # guided
        ...
        assert isinstance(app.scenes.current.inner, AlertConfigScene)  # independent
        ...
        assert result.guided_choices == CORRECT_COMBO_BY_REQUEST
        assert result.independent_choices == CORRECT_COMBO_BY_REQUEST
