...
EVEN_SPLIT = TOTAL_BUDGET // len(CUSTOMER_GROUPS)
...
def test_the_full_lesson_plays_through_all_eight_stages_to_a_result():
    ...
    try:
        finished_results = []
        runner, collected = build_lesson_five_runner(
        ...
        expected_allocation = {group.key: EVEN_SPLIT for group in CUSTOMER_GROUPS}
...
def test_the_budget_divides_evenly_across_every_group_in_whole_steps():
    assert TOTAL_BUDGET % len(CUSTOMER_GROUPS) == 0
