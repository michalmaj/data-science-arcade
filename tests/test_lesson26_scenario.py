...
def test_the_full_lesson_plays_through_all_eleven_stages_to_a_result():
    ...
    try:
        finished_results = []
        runner, collected = build_lesson_twenty_six_runner(app, on_finished=lambda result: finished_results.append(result))
        runner.start()
        ...
        assert isinstance(app.scenes.current.inner, CorrelationScene)  # initial_correlation_pass
        assert app.scenes.current.inner.guided is False
        _pick_requests(app.scenes.current.inner, all_correct=True)

        assert isinstance(app.scenes.current.inner, ComparisonRevealScene)  # strong_association_reveal
        _confirm_reveal(app.scenes.current.inner)

        assert isinstance(app.scenes.current.inner, ComparisonRevealScene)  # confounding_reveal
        _confirm_reveal(app.scenes.current.inner)

        assert isinstance(app.scenes.current.inner, DialogueScene)  # revision_intro
        _play_dialogue_to_the_end(app.scenes.current)

        assert isinstance(app.scenes.current.inner, CorrelationScene)  # revision_correlation_pass
        _pick_requests(app.scenes.current.inner, all_correct=True)
...
def test_the_revision_pass_is_seeded_with_the_initial_passs_own_pick():
    app = _init_app()
    try:
        runner, collected = build_lesson_twenty_six_runner(app, on_finished=lambda result: None)
        runner.start()
        click_through_mission_briefing(app)
        ...
        _confirm_reveal(app.scenes.current.inner)  # strong_association_reveal
        _confirm_reveal(app.scenes.current.inner)  # confounding_reveal
...
def test_a_real_revision_can_correct_a_motivated_initial_pick_to_a_fully_defensible_final_one():
    ...
    try:
        ...
        _pick_requests(app.scenes.current.inner, all_correct=False)  # initial_correlation_pass

        _confirm_reveal(app.scenes.current.inner)  # strong_association_reveal
        _confirm_reveal(app.scenes.current.inner)  # confounding_reveal
        _play_dialogue_to_the_end(app.scenes.current)  # revision_intro

        _pick_requests(app.scenes.current.inner, all_correct=True)  # revision_correlation_pass, corrected this time
...
def test_reveal_computations_never_mutate_the_final_verdict_mirror_state():
    ...
    try:
        ...
        _pick_requests(app.scenes.current.inner, all_correct=True)  # initial_correlation_pass

        pick_actions = [a for a in collected["analytical_context"]["actions"] if a["key"] == "correlation_pick_push_opens_claim"]
        ...
        pick_actions_after_reveals = [a for a in collected["analytical_context"]["actions"] if a["key"] == "correlation_pick_push_opens_claim"]
