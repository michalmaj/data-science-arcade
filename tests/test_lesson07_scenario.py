...
def _pick_a_strategy(scene: SourceBoardScene) -> None:
    first_strategy_key = STRATEGIES[0].key
...
def test_the_full_lesson_plays_through_all_eight_stages_to_a_result():
    ...
    try:
        finished_results = []
        runner, collected = build_lesson_seven_runner(
        ...
        assert isinstance(app.scenes.current.inner, SourceBoardScene)  # guided
        assert app.scenes.current.guided is True
        _pick_a_strategy(app.scenes.current)

        assert isinstance(app.scenes.current.inner, DialogueScene)  # independent intro
        _play_dialogue_to_the_end(app.scenes.current)

        assert isinstance(app.scenes.current.inner, SourceBoardScene)  # independent
        ...
        assert result.guided_strategy == STRATEGIES[0].key
        assert result.independent_strategy == STRATEGIES[0].key
...
def test_there_are_five_strategies_matching_the_spec():
