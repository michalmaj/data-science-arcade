...
from data_science_arcade.lessons.l20_ab_test_commander.checkpoints import CORRECT_FINAL_CHECKPOINT
from data_science_arcade.lessons.l20_ab_test_commander.scenario import DECISION_FIELDS, build_lesson_twenty_runner
...
def _keep_running_to_the_end(scene: CheckpointMonitorScene) -> None:
...
def test_the_full_lesson_plays_through_all_eight_stages_to_a_result():
    ...
    try:
        finished_results = []
        runner, collected = build_lesson_twenty_runner(
        ...
        assert isinstance(app.scenes.current.inner, CheckpointMonitorScene)  # guided
        assert app.scenes.current.guided is True
        _keep_running_to_the_end(app.scenes.current)

        assert isinstance(app.scenes.current.inner, DialogueScene)  # independent intro
        _play_dialogue_to_the_end(app.scenes.current)

        assert isinstance(app.scenes.current.inner, CheckpointMonitorScene)  # independent
        ...
        assert result.guided_final_checkpoint == CORRECT_FINAL_CHECKPOINT
        assert result.independent_final_checkpoint == CORRECT_FINAL_CHECKPOINT
