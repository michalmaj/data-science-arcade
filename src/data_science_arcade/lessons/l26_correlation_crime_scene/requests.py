...
# Three scenarios, each narrowing a different subset of explanations for
# a genuinely real, strong observed correlation - hand-crafted, not
# random, and verified via script before any of this was written. None
# of the three ever claims a formal "coincidence is ruled out" verdict -
# these small, hand-built samples have no sampling model, null model, or
# significance test behind them, so the honest claim a strong observed r
# supports is "a real pattern worth investigating," never a statement
# about the probability of chance. The correct verdict's position varies
# across all three requests (0, 1, 2) so it can't be recognized by
# position alone.
PUSH_OPENS_CLAIM = CorrelationRequest(
    ...
    options=(
        VerdictOption("strong_association_multiple_explanations_remain", "lesson.l26.option.push_opens_claim.strong_association_multiple_explanations_remain", "lesson.l26.explanation.push_opens_claim.strong_association_multiple_explanations_remain"),
...
DARK_MODE_CLAIM = CorrelationRequest(
    ...
    options=(
        ...
        VerdictOption("device_group_is_a_strong_candidate_explanation", "lesson.l26.option.dark_mode_claim.device_group_is_a_strong_candidate_explanation", "lesson.l26.explanation.dark_mode_claim.device_group_is_a_strong_candidate_explanation"),
    ),
)
