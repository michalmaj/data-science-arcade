...
# represents it. Hand-picked, not derived from a formula - see
# decisions/IMPLEMENTATION_STATE.md for the arithmetic this was checked
...
def estimated_problem_rate(sample: pd.DataFrame) -> float:
    if len(sample) == 0:
        return 0.0
    return float(sample["had_problem"].mean())
...
def sample_python_code(frame_key: str, strategy_key: str, budget: int, seed: int, allocation: dict[str, int] | None) -> str:
    ...
    was produced - shown in the Python Mirror once recorded onto a
    ComparisonValue/PipelineStep."""
    ...
    if strategy_key == "simple_random":
        return f"frame = {frame_expr}\nsample = frame.sample(n={budget}, random_state={seed})"
